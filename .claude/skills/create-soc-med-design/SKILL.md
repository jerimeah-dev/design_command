...
!`cat "${CLAUDE_SKILL_DIR}/dart-pitfalls.md"`
...
Read `${CLAUDE_SKILL_DIR}/bootstrap-templates.md` for the exact file contents to use below.
...
3. **Load context into memory** using the Read tool:
   ...
   - `.claude/context/themes.md`
   - `.claude/context/pages.md`
   - `${CLAUDE_SKILL_DIR}/agent-instructions.md`
...
- The **DART/FLUTTER PITFALLS** block (already loaded from the SHARED section above)
- The agent's specific file list and instructions (from `agent-instructions.md`)
...
Read `${CLAUDE_SKILL_DIR}/qa-instructions.md` for the full QA agent spec.
...
- The full QA instructions from `${CLAUDE_SKILL_DIR}/qa-instructions.md`
